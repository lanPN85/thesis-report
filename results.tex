...
\begin{table}[]
    ...
    \begin{tabu} to \textwidth {|c|X[c]|X[c]|X[c]|X[c]|X[c]|X[c]|}
        ...
        $\gamma = 0.5$ & 0.6286 & 0.5681 & 0.4336 & 0.3460 & 0.2944 & 0.1717 \\ \hline
        $\gamma = 1.0$ & 0.6924 & \textbf{0.6327} & 0.4856 & 0.5609 & \textbf{0.4628} & 0.2920 \\ \hline
...
The loss values implies overfitting, as validation loss does not decrease past epoch 17, while training loss improves steadily. This can be partially attributed to the high variance in the dataset. As such, the model performs rather poorly on our test examples, with a test \gls{ap} of 0.4856 at IoU=0.5. Further adjustments can be made to better adapt RetinaNet to text boxes, however we were not able to dive too deeply in these techniques during this case study.
