...
\begin{algorithm}[bth]
	...
	\Input{$x$: a vector of input features\\
		$L = ((W^{(1)}, b^{(1)}, g^{(1)}),...,(W^{(m)}, b^{(m)}, g^{(m)}))$: the network's layers
		}
	\Output {
		$h^{(m)}$: the network's prediction\\
		$Z = (z^{(1)},...,z^{(m)})$: the output at each layer
	}
...
\begin{algorithm}[bth]
	...
	\Output {
		$\Delta = (\delta^{(1)},...,\delta^{(m)})$: the gradients at each layer
	}
