...
In our final experiment, we train the same network to convergence using BK.Synapse, and evaluate the final model's performance on the \gls{icdar} Challenge task. We maintain the same parameters as in Table \ref{tab:train_params}, but set the number of epochs to 50. The network is trained on both N1 and N2 with GPU acceleration.

We sample 3 different values of $\gamma$ for Focal Loss: 0.5, 1.0, and 2.0. AP is used as our evaluation metric. The final training results are shown in Table \ref{tab:ap}.
