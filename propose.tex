...
The training script is invoked using \lstinline{mpirun}, from a subprocess spawned by the API server when the start endpoint is called, making it the MPI monitor node. The server collects the selected worker nodes' IPs and generates a command as follow:
