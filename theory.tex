...
Non-Maximum Suppression (NMS) is a procedure that takes a series of overlapping bouding boxes as input, and outputs the pruned list of non-overlapping boxes. NMS achieves this by prioritizing boxes with the lowest bottom-right corner, and removing any overlapping box over a predefined threshold.
...
Several metrics are introduced to measure object detectors' performance. Intersection-over-Union (IoU) is commonly used to measure box location accuracy. Given a predicted bounding box and a ground truth box, IoU is computed as the ratio between their intersection area and their union area:
\begin{equation}
