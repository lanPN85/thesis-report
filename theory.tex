...
In the first perceptron, $y \in \{-1, 1\}$, which corresponds to a binary classification of "no" or "yes". Given a training dataset with examples $X_a = (X_1, ..., X_n)$ and labels $Y = (y_1, ..., y_n)$, the problem becomes finding a set of values for $W$ such that $\sum_{i=0}^n(y_i = f(W \cdot X_i)) \rightarrow max$.

We can model the way this perceptron classify examples as creating a decision boundary on the feature space (Figure \ref{fig:decision_bound}).

It can be seen that a value of 1 is added to the input, along with an extra weight $w_0$. This is the bias value, which allows the perceptron to model constant offsets in the output function. In other words, it allows the decision boundary to move along the axes.

\begin{figure}[htp]
	\centering
	\includegraphics[width=0.7\textwidth]{decision_bound.png}
	\caption{Example of a decision boundary on a 2D feature space}
